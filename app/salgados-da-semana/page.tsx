...
import { Minus, Plus, Send, ShoppingBasket, Cookie } from "lucide-react";
...
type SalgadoPublico = { id: string; nome: string; preco: number; quantidade: number };
type DadosPublicos = { id: number; whatsappNumber?: string | null; destinoWhatsApp?: string; salgados: SalgadoPublico[] };
...
export default function SalgadosDaSemanaPage() {
  ...
  useEffect(() => {
    ...
    fetch(apiUrl(path), { cache: "no-store" })
      .then(async (res) => {
        ...
        if (!res.ok) throw new Error(json?.message || "Erro ao carregar salgados.");
        setDados(json);
      })
      .catch((e) => setErro(e?.message || "Erro ao carregar salgados."));
  }, []);

  const salgadosVisiveis = useMemo(() => (dados?.salgados || []).filter((item) => item.quantidade > 0), [dados]);
  const total = useMemo(() => Object.values(quantidades).reduce((soma, quantidade) => soma + Number(quantidade || 0), 0), [quantidades]);

  function alterar(item: SalgadoPublico, delta: number) {
  ...
  async function enviar() {
    ...
    const itens = dados.salgados
      .filter((item) => Number(quantidades[item.id] || 0) > 0)
      .map((item) => ({ salgadoId: Number(item.id), quantidade: Number(quantidades[item.id]), nome: item.nome }));
    ...
    try {
      const res = await fetch(apiUrl("/public/pedidos"), {
        ...
        body: JSON.stringify({ cardapioId: dados.id, tipoPedido: "SALGADOS", origem: dados.destinoWhatsApp || "principal", nome, telefone, tamanhoLabel: "SALGADOS", itens }),
      });
      ...
      const mensagem = `Olá, fiz um novo pedido de salgados pelo site!\nPedido #${json.id}`;
    ...
    <main className="min-h-screen bg-[#eef7f8] px-3 py-5 text-[#15332f] sm:px-6">
      <div className="mx-auto max-w-4xl rounded-[28px] border border-sky-100 bg-white p-4 shadow-xl sm:p-8">
        <header className="flex items-center gap-4 border-b pb-5">
          <div className="grid h-14 w-14 place-items-center rounded-2xl bg-amber-100 text-amber-700"><Cookie className="h-8 w-8" /></div>
          <div><p className="text-xs font-black uppercase tracking-widest text-amber-700">Estoque disponível</p><h1 className="text-3xl font-black">Salgados da semana</h1></div>
        </header>
        ...
        <section className="mt-6 divide-y overflow-hidden rounded-2xl border">
          {salgadosVisiveis.map((item) => (
            <div key={item.id} className="grid grid-cols-[1fr_auto] items-center gap-4 p-4">
              <div>
                <p className="font-bold text-slate-900">{item.nome}</p>
                <p className="mt-1 text-xs font-semibold text-[#60746f]">R$ {Number(item.preco || 0).toFixed(2).replace(".", ",")} · estoque {item.quantidade}</p>
              </div>
          ...
          {dados && salgadosVisiveis.length === 0 ? <p className="p-8 text-center text-[#60746f]">Nenhum salgado disponível no momento.</p> : null}
        </section>
        <section className="mt-6 space-y-4 rounded-2xl border p-4">
          <div className="flex items-center gap-2 font-black"><ShoppingBasket className="h-5 w-5" /> Seu pedido: {total} salgado(s)</div>
          ...
          <Button className="h-12 w-full bg-[#c24f2f] font-black text-white hover:bg-[#a94329]" onClick={enviar} disabled={enviando || !total || !nome.trim() || !telefone.trim()}><Send className="mr-2 h-5 w-5" />{enviando ? "Enviando..." : "Enviar pedido de salgados"}</Button>
        </section>
      </div>
    </main>
  );
